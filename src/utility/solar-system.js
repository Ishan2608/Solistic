...
// Orbit controls (imported inline since CDN path was causing issues)
class OrbitControls {
...
// Mouse button state
this.isRotating = false; // Track if the left mouse button is pressed
...
this.domElement.addEventListener('pointerleave', this.onPointerUp.bind(this)); // Stop rotating if the mouse leaves the canvas
this.domElement.addEventListener('wheel', this.onMouseWheel.bind(this)); // Handle zooming
...
if (event.button === 0) { // Left mouse button
    this.isRotating = true; // Start rotating
    this.handleMouseDownRotate(event);
}
}

onPointerMove(event) {
if (!this.enabled || !this.isRotating) return; // Rotate only if the left button is pressed
this.handleMouseMoveRotate(event);
this.update();
}

onPointerUp() {
this.isRotating = false; // Stop rotating when the left mouse button is released
}
...
// Adjust zoom scale based on wheel delta
...
function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (planet) {
            ...
            infoTitle.textContent = planet.data.name;
            infoContent.textContent = planet.data.description;
